...
// ---------------------------------------------------------------------------
// Scan — detect projects from providers and persist to SQLite
// ---------------------------------------------------------------------------
...
export function scan(
  ...
  // Purge previously-stored projects whose path is now considered obsolete
  // (e.g. eval workspace artifacts that polluted older scans).
  const allRows = db.select({ id: dbSchema.projects.id, projectPath: dbSchema.projects.projectPath }).from(dbSchema.projects).all();
  ...
  // Get dismissed project IDs to exclude them
