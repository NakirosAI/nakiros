          ...
          <>
            ...
            <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <Card
                ...
                tone={
                  stats.compactionRate >= 0.5
                    ? 'danger'
                    : stats.compactionRate >= 0.3
                      ? 'warning'
                      : 'muted'
                }
              />
    ...
    <div>
      <div className="flex h-6 w-full overflow-hidden rounded border border-[var(--line)]">
        {h > 0 && (
          <div
            style={{ width: `${h}%` }}
            className="flex items-center justify-center bg-[var(--success)] text-[10px] font-semibold text-white"
          >
            {h >= 8 ? `${Math.round(h)}%` : ''}
          </div>
        )}
        {w > 0 && (
          <div
            style={{ width: `${w}%` }}
            className="flex items-center justify-center bg-[var(--warning)] text-[10px] font-semibold text-black"
          >
            {w >= 8 ? `${Math.round(w)}%` : ''}
          </div>
        )}
        {d > 0 && (
          <div
            style={{ width: `${d}%` }}
            className="flex items-center justify-center bg-[var(--danger)] text-[10px] font-semibold text-white"
          >
            {d >= 8 ? `${Math.round(d)}%` : ''}
          </div>
        )}
