...
/**
 * Per-rule detail screen — 3-tab layout aligned with ClaudeMdScreen:
 *
 ...
 *
 * Calqué ligne par ligne sur ClaudeMdScreen; uses the `rules:read`,
 * `rules:save`, `rules:delete` and `rules:listAudits` / `rules:readAudit`
 * IPC channels.
 */
export default function RuleDetailScreen({
  ...
  // ── Audit history ────────────────────────────────────────────────────────
  const [audits, setAudits] = useState<RulesAuditHistoryEntry[]>([]);
  ...
  useEffect(() => {
    ...
    window.nakiros
      .readRulesAudit(selectedAudit.path)
      .then((md) => {
        if (cancelled) return;
        setAuditContent(md ?? '');
      })
  ...
  const handleLaunchRun = async (mode: RulesRunMode) => {
    ...
    try {
      ...
      // comes back to the Audit tab (no polling needed — RunScreen drives it).
    } catch (err) {
      setErrorBanner({
        code: 'launch-failed',
        message: err instanceof Error ? err.message : String(err),
  ...
  // Build a human-readable rule label: replace internal slashes with " / " for readability.
  const ruleDisplayName = ruleName.replace(/\//g, ' / ');
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden font-n-sans">
      ...
      <div className="flex flex-wrap items-center gap-3.5 border-b border-n-border-subtle px-7 py-3.5">
        ...
        <strong className="font-n-mono text-[14px] font-medium text-n-fg">{ruleDisplayName}</strong>
        <span
...
function EditTab({
  ...
  // Extract paths: glob from frontmatter `paths:` field.
  const pathsGlobs = useMemo(() => extractPathsGlobs(body), [body]);
  ...
  return (
    <div className="flex flex-1 overflow-hidden" style={{ height: '100%' }}>
      ...
      <aside className="hidden w-72 flex-shrink-0 flex-col gap-4 overflow-auto border-l border-n-border-subtle bg-n-surface px-4 py-4 lg:flex">
        ...
        {pathsGlobs.length > 0 && (
          <SidebarSection title={t('detail.editTab.paths')}>
        ...
        <SidebarSection title={t('detail.editTab.ruleName')}>
          <span className="break-all font-n-mono text-[11.5px] text-n-muted">{ruleName}</span>
        </SidebarSection>
      </aside>
    </div>
  );
...
function AuditTab({
  ...
  ruleName: string;
  onOpenRunTab?: OpenRunTabCallback;
  launchingMode: RulesRunMode | null;
  onLaunchFix(): void;
  t: (key: string, opts?: Record<string, unknown>) => string;
}) {
  if (audits.length === 0) {
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="font-n-mono text-[18px] text-n-fg">
              Audit — <span className="text-n-accent">{ruleName}</span>
            </div>
          </div>
...
// ── Paths glob extractor ───────────────────────────────────────────────────

/**
 * Extracts `paths:` glob values from YAML frontmatter.
 * Handles both inline (`paths: "src/**"`) and block list (`paths:\n  - src/**`) forms.
 */
function extractPathsGlobs(content: string): string[] {
  // Only look in the first 50 lines (frontmatter zone)
  ...
  // Inline: `paths: "src/**"` or `paths: src/**`
  const inlineMatch = fm.match(/^paths:\s*"?([^"\n]+)"?\s*$/m);
  if (inlineMatch?.[1]) return [inlineMatch[1].trim()];

  // Block list: `paths:\n  - pattern`
  const blockStart = fm.match(/^paths:\s*$/m);
  if (blockStart) {
    const fmLines = fm.split('\n');
    const pathsIdx = fmLines.findIndex((l) => /^paths:\s*$/.test(l));
    if (pathsIdx >= 0) {
      const items: string[] = [];
      for (let i = pathsIdx + 1; i < fmLines.length; i++) {
