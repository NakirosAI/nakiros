...
Composes the shared run library: `RunControlHeader` + `AgentActivityFeed` + `HumanInteractionPanel` + `RunErrorBanner`. The conversation/report tab switcher and the markdown report panel are the only audit-specific surfaces.
