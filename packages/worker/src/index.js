...
import { buildWorkspaceMembershipList, canManageWorkspaceMemberships, hasManageableWorkspaceMember, isWorkspaceRole, } from './workspace-membership.js';
import { decryptProviderSecret, encryptProviderSecret, parseBindWorkspaceCredentialInput, parseCreateProviderCredentialInput, parseSetWorkspaceDefaultInput, parseUpdateProviderCredentialInput, toProviderCredentialSummary, toWorkspaceBinding, } from './provider-credentials.js';
...
import { fetchJiraStoryState, detectConflict } from './pm-sync.js';
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (request.method === 'POST' && url.pathname === '/orgs') {
            ...
            if (!slug)
                return json({ error: 'Organization slug is required' }, 400);
            ...
            if (existing)
                return json({ error: 'This identifier is already taken — choose another' }, 409);
        ...
        if (orgMembersMatch && request.method === 'GET') {
            ...
            const patched = members.map((m) => m.userId === auth.userId && !m.email && auth.email ? { ...m, email: auth.email } : m);
        ...
        if (orgMembersMatch && request.method === 'POST') {
            ...
            if (env.RESEND_API_KEY) {
                ...
                ctx.waitUntil(sendInvitationEmail(env.RESEND_API_KEY, {
                    to: email,
                    orgName,
        ...
        if (orgMemberMatch && request.method === 'DELETE') {
            const [, orgId, targetUserId] = orgMemberMatch;
            if (!(await storage.isOrgAdmin(orgId, auth.userId))) {
                return json({ error: 'Forbidden — admin only' }, 403);
            }
        ...
        if (invitationMatch && request.method === 'DELETE') {
            const [, orgId, invId] = invitationMatch;
            if (!(await storage.isOrgAdmin(orgId, auth.userId))) {
                return json({ error: 'Forbidden — admin only' }, 403);
            }
        ...
        if (request.method === 'GET' && url.pathname === '/workspaces/resolve') {
            ...
            if (!repoPath)
                return json({ error: 'path query param required' }, 400);
            const all = await storage.readWorkspaces();
            for (const workspace of all) {
                if (!(await canAccessWorkspace(storage, workspace, auth)))
                    continue;
                const match = workspace.repos.some((repo) => repoPath === repo.localPath || repoPath.startsWith(repo.localPath + '/'));
                if (match)
        ...
        if (request.method === 'POST' && url.pathname === '/provider-credentials') {
            try {
                ...
                if (!created)
                    return json({ error: 'Credential not found after creation' }, 500);
        ...
        if (providerCredentialMatch && request.method === 'PUT') {
            ...
            if (existing.ownerId !== deriveCredentialOwnerId(auth))
                return json({ error: 'Forbidden' }, 403);
            try {
                const input = parseUpdateProviderCredentialInput(existing.provider, await request.json());
        ...
        if (mcpMatch) {
            ...
            if (!workspace)
                return json({ error: `Workspace '${workspaceId}' not found` }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
                return json({ error: 'Forbidden' }, 403);
        ...
        if (wsMatch && request.method === 'DELETE') {
            const workspaceId = wsMatch[1];
            const workspace = await storage.readWorkspace(workspaceId);
            if (workspace && !(await canAccessWorkspace(storage, workspace, auth)))
                return json({ error: 'Forbidden' }, 403);
        ...
        if (workspaceProviderBindMatch && request.method === 'POST') {
            ...
            try {
                ...
                if (!credential)
                    return json({ error: 'Credential not found' }, 404);
                if (credential.ownerId !== deriveWorkspaceScopeOwnerId(workspace, auth))
                    return json({ error: 'Forbidden' }, 403);
                if (credential.provider !== input.provider)
                    return json({ error: 'Credential provider mismatch' }, 400);
                if (credential.revokedAt)
                    return json({ error: 'Credential is revoked' }, 400);
        ...
        if (workspaceProviderDefaultMatch && request.method === 'PUT') {
            ...
            try {
                ...
                if (!target)
                    return json({ error: 'Credential is not bound to this workspace' }, 404);
                const credential = await storage.readProviderCredential(input.credentialId);
                if (!credential)
                    return json({ error: 'Credential not found' }, 404);
                if (credential.revokedAt)
        ...
        if (workspaceProviderCredentialMatch && request.method === 'DELETE') {
            const [, workspaceId, credentialId] = workspaceProviderCredentialMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (workspaceMemberMatch && request.method === 'PUT') {
            const [, workspaceId, targetUserId] = workspaceMemberMatch;
            const context = await resolveWorkspaceMembershipContext(storage, workspaceId, auth, { backfillManager: true });
            if (context.errorResponse)
                return context.errorResponse;
            ...
            if (request.method === 'GET') {
                ...
                for (const [name, repoCtx] of Object.entries(ctx.repos ?? {})) {
                    reposMeta[name] = {
                        ...
                        files: ['architecture', 'stack', 'conventions', 'api', 'llms']
        ...
        if (globalFileMatch && request.method === 'GET') {
            const [, workspaceId, fileName] = globalFileMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (repoCtxMatch) {
            const [, workspaceId, repoName] = repoCtxMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
            ...
            if (request.method === 'GET') {
                ...
                if (!repoCtx)
                    return json({ error: 'No context for this repo' }, 404);
                return json(repoCtx);
            }
        ...
        if (collabMatch && request.method === 'GET') {
            ...
            if (!collab)
                return json({ error: 'Collab not found' }, 404);
            return json(collab);
        }
        ...
        if (taskDependencyMatch && request.method === 'DELETE') {
            const [, workspaceId, storyId, taskId, depId] = taskDependencyMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (taskDependenciesMatch && request.method === 'POST') {
            const [, workspaceId, storyId, taskId] = taskDependenciesMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
            ...
            if (!depBody.dependsOnTaskId)
                return json({ error: 'dependsOnTaskId is required' }, 400);
            if (taskId === depBody.dependsOnTaskId)
                return json({ error: 'A task cannot depend on itself' }, 400);
        ...
        if (taskMatch && request.method === 'PATCH') {
            const [, workspaceId, storyId, taskId] = taskMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (tasksMatch) {
            const [, workspaceId, storyId] = tasksMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (storyMatch && request.method === 'PATCH') {
            const [, workspaceId, storyId] = storyMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (storiesMatch) {
            const [, workspaceId] = storiesMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (sprintMatch && request.method === 'PATCH') {
            const [, workspaceId, sprintId] = sprintMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
            ...
            let sprintPatch;
            try {
                sprintPatch = (await request.json());
            }
            catch {
                return json({ error: 'Invalid JSON body' }, 400);
            }
        ...
        if (sprintsMatch) {
            const [, workspaceId] = sprintsMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (epicMatch && request.method === 'PATCH') {
            const [, workspaceId, epicId] = epicMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
            ...
            if (!epic || epic.workspaceId !== workspaceId)
                return json({ error: 'Epic not found' }, 404);
            let epicPatch;
            try {
                epicPatch = (await request.json());
            }
            catch {
                return json({ error: 'Invalid JSON body' }, 400);
            }
        ...
        if (actionsMatch && request.method === 'POST') {
            const [, workspaceId, tool] = actionsMatch;
            const workspace = await storage.readWorkspace(workspaceId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
            if (!(await canAccessWorkspace(storage, workspace, auth)))
        ...
        if (pmImportMatch && request.method === 'POST') {
            ...
            try {
                switch (provider) {
                    case 'jira':
                        result = await runJiraImport(importContext, storage, env);
                        break;
                }
            }
            catch (err) {
        ...
        if (pmSyncMatch && request.method === 'POST') {
            ...
            const syncResult = { synced: 0, conflicts: 0, errors: 0 };
        ...
        if (wsArtifactVersionsMatch && request.method === 'GET') {
            const [, wsId, encodedPath] = wsArtifactVersionsMatch;
        ...
        if (wsArtifactVersionsMatch && request.method === 'POST') {
            ...
            if (!body.content || !body.artifactType)
                return json({ error: 'content and artifactType required' }, 400);
        ...
        if (wsArtifactVersionMatch && request.method === 'GET') {
            const [, wsId, encodedPath, versionStr] = wsArtifactVersionMatch;
            const artifactPath = decodeURIComponent(encodedPath);
            const workspace = await storage.readWorkspace(wsId);
            if (!workspace)
                return json({ error: 'Workspace not found' }, 404);
...
export function canAccess(workspace, auth) {
    if (!workspace.ownerId)
        return true; // migration: no owner yet
    return workspace.ownerId === auth.orgId || workspace.ownerId === auth.userId;
}
export function deriveWorkspaceOwnerId(existingOwnerId, auth) {
    return existingOwnerId ?? auth.userId;
}
async function canAccessWorkspace(storage, workspace, auth) {
    if (!workspace.ownerId)
        return true;
...
export async function resolveWorkspaceOwnerId(storage, existingOwnerId, requestedOwnerId, auth) {
    if (!existingOwnerId) {
    ...
    if (existingOwnerId !== auth.userId) {
        return {
            ...
            shouldSeedOwnerMembership: false,
            errorResponse: json({ error: 'Workspace ownership can only be transferred from a personal workspace to one of your organizations' }, 400),
        };
    }
...
async function seedWorkspaceOwnerMembership(storage, workspaceId, userId) {
    const now = new Date().toISOString();
...
function isOwnedByCurrentScope(workspace, auth) {
    if (workspace.ownerId)
        return workspace.ownerId === auth.userId;
    return canAccess(workspace, auth);
}
function deriveCredentialOwnerId(auth) {
    return auth.orgId ?? auth.userId;
}
function deriveWorkspaceScopeOwnerId(workspace, auth) {
    return workspace.ownerId ?? deriveCredentialOwnerId(auth);
}
async function resolveWorkspaceMembershipContext(storage, workspaceId, auth, options) {
    const workspace = await storage.readWorkspace(workspaceId);
    if (!workspace) {
        return {
...
async function buildProviderCredentialSummaries(storage, ownerId) {
...
async function buildWorkspaceProviderCredentialsPayload(storage, workspace, ownerId) {
...
async function buildProviderCredentialDeleteImpact(storage, credentialId) {
