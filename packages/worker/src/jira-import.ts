...
import type { PmImportContext, PmImportResult } from './pm-import.js';
import type { EpicRow, StoryRow } from './types.js';
...
interface JiraIssueFields {
  ...
  customfield_10020?: Array<{ name: string }> | null; // sprints
  customfield_10014?: string | null; // epic link (classic)
...
export function extractAdfText(adf: unknown): string | null {
...
export function mapJiraStatus(s: string): StoryRow['status'] {
  const l = s.toLowerCase();
  if (['done', 'closed', 'resolved'].includes(l)) return 'done';
  if (['in progress', 'in development'].includes(l)) return 'in_progress';
  if (['in review', 'code review', 'testing'].includes(l)) return 'in_review';
  if (['to do', 'open', 'selected for development'].includes(l)) return 'todo';
  return 'backlog';
}

export function mapJiraEpicStatus(s: string): EpicRow['status'] {
  const l = s.toLowerCase();
  if (['done', 'closed', 'resolved'].includes(l)) return 'done';
  if (['in progress', 'in development'].includes(l)) return 'in_progress';
  return 'backlog';
...
export function mapJiraPriority(s: string | undefined): StoryRow['priority'] {
  const l = (s ?? '').toLowerCase();
  if (['high', 'highest', 'critical', 'blocker'].includes(l)) return 'high';
  if (['low', 'lowest', 'trivial'].includes(l)) return 'low';
  return 'medium';
}

// ─── Issue creation helpers (used by pm-push.ts) ─────────────────────────────

export interface AdfNode {
  type: string;
  content?: Array<{ type: string; text?: string } | AdfNode>;
}
...
export function plainTextToAdf(text: string | null): AdfDocument {
...
export function mapNakirosPriorityToJira(priority: string | null | undefined): string {
  if (priority === 'high') return 'High';
  if (priority === 'low') return 'Low';
  return 'Medium';
}

export function buildJiraIssuePayload(story: StoryRow, projectKey: string): JiraCreateIssuePayload {
...
function getEpicKey(issue: JiraIssue): string | null {
  return issue.fields.parent?.key ?? (issue.fields.customfield_10014 as string | null) ?? null;
}

function getCurrentSprintName(issue: JiraIssue): string | undefined {
...
type DbTx = Parameters<D1Storage['db']['transaction']>[0] extends (tx: infer T) => unknown ? T : never;

async function upsertEpicInTx(
  tx: DbTx,
  workspaceId: string,
  issue: JiraIssue,
  ...
  const existing = (await tx
    ...
    .where(and(
      ...
      eq(schema.epics.externalId, issue.key),
      eq(schema.epics.externalSource, 'jira'),
    ))
    .all())[0];
...
async function upsertStoryInTx(
  tx: DbTx,
  workspaceId: string,
  issue: JiraIssue,
  ...
  const existing = (await tx
    ...
    .where(and(
      ...
      eq(schema.stories.externalId, issue.key),
      eq(schema.stories.externalSource, 'jira'),
    ))
    .all())[0];
...
export async function runJiraImport(
  context: PmImportContext,
  storage: D1Storage,
  _env: ProviderCredentialsEnv,
  ...
  const jiraEpics = await fetchAllJiraIssues(
    baseUrl, authHeader,
    `project=${projectKey} AND issuetype=Epic`,
    epicFields,
  );
  const jiraStories = await fetchAllJiraIssues(
    baseUrl, authHeader,
    `project=${projectKey} AND issuetype in (Story, Task, Bug, "Sub-task")`,
    storyFields,
  );
  ...
  await storage.db.transaction(async (tx) => {
    for (const issue of jiraEpics) {
      const { created } = await upsertEpicInTx(tx as DbTx, workspaceId, issue);
      created ? epicCount++ : updatedCount++;
    }

    // Build epicMap WITHIN transaction for consistency
    const epicRows = await (tx as DbTx)
    ...
    for (const issue of jiraStories) {
      const { created } = await upsertStoryInTx(tx as DbTx, workspaceId, issue, epicMap, sprintMap);
