...
export function parseProviderCredentialMetadata(provider, input) {
    switch (provider) {
        case 'jira': {
...
export function parseCreateProviderCredentialInput(input) {
...
export function parseUpdateProviderCredentialInput(provider, input) {
...
export function parseBindWorkspaceCredentialInput(input) {
...
export function parseSetWorkspaceDefaultInput(input) {
...
async function getMasterKey(env) {
    if (!cachedMasterKey) {
        ...
        if (!binding) {
            throw new Error('Provider credentials master key is not configured. Bind PROVIDER_CREDENTIALS_MASTER_KEY via Cloudflare Secrets Store before creating credentials.');
        }
        const secretPromise = typeof binding === 'string'
            ? Promise.resolve(binding)
            : typeof binding.get === 'function'
                ? binding.get()
                : Promise.reject(new Error('Provider credentials master key binding is invalid. Expected a Cloudflare Secrets Store binding or Worker secret.'));
...
export async function encryptProviderSecret(env, secret) {
    const key = await getMasterKey(env);
    const iv = crypto.getRandomValues(new Uint8Array(12));
    const encodedSecret = new TextEncoder().encode(secret);
    const encrypted = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, encodedSecret));
...
export async function decryptProviderSecret(env, row) {
    const key = await getMasterKey(env);
    const combined = new Uint8Array([
        ...decodeBase64(row.secretCiphertext),
        ...decodeBase64(row.authTag),
    ]);
    const decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: decodeBase64(row.iv) }, key, combined);
    return new TextDecoder().decode(decrypted);
}
export function toProviderCredentialSummary(row, usage) {
    return {
        id: row.id,
...
export function toWorkspaceBinding(row, credential) {
    return {
