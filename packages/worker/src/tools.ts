...
import type { CollabMessage, CollabSession, RepoContext, StoredWorkspace } from './types.js';
...
export function registerWorkspaceTools(server: McpServer, workspace: StoredWorkspace, storage: IStorage): void {
  ...
  server.tool(
    ...
    {},
    async () => ({
      content: [{ type: 'text' as const, text: workspace.context?.global ?? 'No global context generated yet.' }],
    })
  );
  ...
  server.tool(
    ...
    {},
    async () => ({
      content: [{ type: 'text' as const, text: workspace.context?.product ?? 'No product context generated yet.' }],
    })
  );
